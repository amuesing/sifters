...
def create_midi(notes_per_step, filename, velocities, duration_multiplier, time_signature):
...
def create_ensemble_midi(ensemble_tracks, filename='ensemble_prime'):
    ...
    for i, (instrument_name, notes_per_step, velocities, duration_multiplier, time_signature) in enumerate(ensemble_tracks):
        track = mido.MidiTrack()
...
def process_instrument(config, ensemble_tracks):
    ...
    for t_name in all_transformations:
        ...
        create_midi(notes_per_step, filename, velocities, duration_multiplier, time_signature)

        if t_name == 'prime':
            ensemble_tracks.append((instrument_name, notes_per_step, velocities, duration_multiplier, time_signature))
...
def main():
    ...
    for config in INSTRUMENT_CONFIGS:
        process_instrument(config, ensemble_tracks)
