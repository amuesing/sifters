...
def accent_voicing(binary, accent_binaries, profile, root_note, pitch_period):
...
def verify(voices, periods, total_ticks, note_layers, base_binaries):
    ...
    for name, _, velocities, step_ticks in voices:
        ...
        wrong = [(o, p) for o, _, p, _ in notes
                 if p != lattice_pitch((o // step_ticks) % period, period)]
...
def main():
    ...
    diagonal = check_pitch_lattice(note_layers[INSTRUMENT_CONFIGS[0]['name']])
