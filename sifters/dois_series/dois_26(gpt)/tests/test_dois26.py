"""Tests for dois_26(gpt): two pitch modes from one rhythm, and not tied to one sieve.
...
Nothing here touches dois_26(gpt)/mid.
"""
import json
import re
import shutil
...
class SameResult(unittest.TestCase):
    ...
    def test_static_mode_is_the_drumrack_version_note_for_note(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for f in FILES:
                self.assertEqual(notes(root / 'mid' / f'dois_26(gpt)_static_{f}.mid'),
    ...
    def test_lattice_shares_the_rhythm_and_velocities_of_static(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run(root).returncode, 0)
            for f in FILES:
                a = notes(root / 'mid' / f'dois_26(gpt)_static_{f}.mid')
                b = notes(root / 'mid' / f'dois_26(gpt)_lattice_{f}.mid')
    ...
    def test_lattice_pitches_and_canon(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            at = {}
            for voice in 'ABCD':
                (track,) = notes(root / 'mid' / f'dois_26(gpt)_lattice_{voice}_prime.mid')
    ...
    def test_each_voice_sounds_one_static_pitch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for voice, pitch in (('A', 36), ('B', 37), ('C', 38), ('D', 39)):
                (track,) = notes(root / 'mid' / f'dois_26(gpt)_static_{voice}_prime.mid')
