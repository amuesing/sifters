...
def accent_field(cfg, base_binaries, note_layers, units, parity, residue_source=None):
    ...
    accent_dict = dict(config.WEATHER, **{label: expression})
...
def report_span_suggestion(fields, note_layers, field):
    ...
    current = {name: duplicate_passes(f['velocities'], note_layers[name])
               for name, f in fields.items()}
    print("\n  --suggest-span")
    print(f"    configured SPAN_RESIDUE_SOURCE = {tuple(config.SPAN_RESIDUE_SOURCE)}: "
...
def write_files(voices, periods, total_ticks, meters, ensemble_meter, parity, prefix):
    ...
    stamp = (f"{prefix} cfg={fingerprint} "
             ...
             f"weather={'+'.join(config.WEATHER)} sieve={config.INSTRUMENT_CONFIGS[0]['sieve']}")
...
def prepare_accents(field, note_layers):
    ...
    """
    fields = {cfg['name']: field(cfg) for cfg in config.INSTRUMENT_CONFIGS}
    for cfg in config.INSTRUMENT_CONFIGS:
        got = fields[cfg['name']]
...
def main():
    ...
    fields = prepare_accents(field, note_layers)
