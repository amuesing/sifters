"""dois_20 — sieve to MIDI, for the Moog Grandmother.
...
New in dois_20 — PITCH IS STATIC, and that is the whole of it:

  * one fixed note per voice (36-39), never moving. This version is for realising the
    piece on a Moog Grandmother, which cannot take pitch as a CV source, so the sieve
    is carried by RHYTHM and VELOCITY alone — the two things that instrument can act
    on. Rhythm, accents, weather, parity, meter and channels are exactly dois_18's;
    only the pitch of each note differs, and verify() asserts each voice sounds its one
    note. The lattice and the canon's pitch reporting are gone with it.
  * nothing is lost: dois_15-dois_18 and dois_18(gpt)/dois_19(gpt) derived pitch from
    the sieve four different ways and all of it stands. See CONTEXT.md, "Pitch work,
    parked". No part of the rhythm or accent structure ever depended on it.

Carried forward from dois_18:
...
def verify(voices, periods, total_ticks, note_layers, base_binaries):
    ...
    for name, _, velocities, step_ticks in voices:
        ...
        # Pitch is STATIC here: every note of a voice must be its one configured note,
        # read back from the FILE. If pitch ever moves again, this is the check that
    ...
    # The canon is still here in the RHYTHM — C is A shifted 13 steps, asserted by
    # check_derivations() — but with pitch static there is no pitch canon to measure.
    # dois_15 through dois_19(gpt) reported the interval it produced; see CONTEXT.md,
    # "Pitch work, parked", if that ever comes back.
...
def main():
    ...
    # Static pitch: one note per voice, checked before clear_our_outputs() so a bad
    # value fails with the previous files still in place.
    for cfg in INSTRUMENT_CONFIGS:
        value = cfg['pitch']
    ...
    if len({c['pitch'] for c in INSTRUMENT_CONFIGS}) != len(INSTRUMENT_CONFIGS):
        raise ValueError("two voices share a pitch; they would be indistinguishable "
                         "in the merged file")
    print("\n  Pitch: STATIC, one note per voice — "
          + ", ".join(f"{c['name']}={c['pitch']}" for c in INSTRUMENT_CONFIGS)
          + ". The sieve is carried by rhythm and velocity alone.")
