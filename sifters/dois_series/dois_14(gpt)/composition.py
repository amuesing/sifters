...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(description='Verified sieve music; ordinary MIDI folders for Ableton.')
    ...
    try:
        ...
        plans = [(name, engine.build_plan(config.settings(name))) for name in names]
        root = args.output_dir if args.output_dir is not None else config.OUTPUT_DIR
        if args.diagnostics:
        ...
        for name, plan in plans:
            ...
            for v in plan.voices:
                print(f'  {v.name}: {v.unit} ticks/step; {len(v.velocities)//len(v.layer)} distinct passes; {len(v.notes)} notes')
            if args.verify_only:
