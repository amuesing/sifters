...
from accents import (accent_voicing, create_accent_binaries, duplicate_passes,
                     generate_velocity_profile, span_accent, voice_span)
...
def search_residue_source(accent_field, note_layers, largest=16, most=5):
    ...
    for size in range(2, most + 1):
        for candidate in itertools.combinations(range(largest), size):
            try:
                if all(not duplicate_passes(accent_field(cfg, candidate)[-1],
                                            note_layers[cfg['name']])
...
def check_static_pitch():
    """One whole-number MIDI note per voice, all different — checked before writing."""
    for cfg in INSTRUMENT_CONFIGS:
        value = cfg['pitch']
...
def accent_field(cfg, base_binaries, note_layers, units, parity, residue_source=None):
    """One voice's accents and the velocity of each of its steps.
    ...
    notes_per_step, velocities = accent_voicing(binary_full, accent_bins, profile,
                                                cfg['pitch'])
    return label, expression, accent_dict, span, profile, notes_per_step, velocities


def build_voices(field, note_layers):
    """Each voice's sounding steps and velocities, with its accents printed."""
    voices = []
    for cfg in INSTRUMENT_CONFIGS:
        name = cfg['name']
        _, _, _, span, profile, notes_per_step, velocities = field(cfg)
        ...
        voices.append((name, notes_per_step, velocities, get_step_ticks(cfg)))
    return voices
...
def write_files(voices, periods, total_ticks, meters, ensemble_meter, parity):
...
def main():
    ...
    def field(cfg, residue_source=None):
        return accent_field(cfg, base_binaries, note_layers, units, parity, residue_source)

    for cfg in INSTRUMENT_CONFIGS:
        _, expression, accent_dict, *_ = field(cfg)
        cfg['accent_dict'] = accent_dict
        print(f"  {cfg['name']}: span accent derived — modulus {true_period(expression)}, "
              f"{expression}")

    check_static_pitch()

    print()
    voices = build_voices(field, note_layers)

    if '--suggest-span' in sys.argv:
        report_span_suggestion(voices, note_layers, field)
        return
    require_distinct_passes(voices, note_layers, field)
    ...
    meters, ensemble_meter = choose_meters(voices, periods, note_layers, total_ticks)
