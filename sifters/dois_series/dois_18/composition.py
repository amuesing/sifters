...
New in dois_18 — one musical change, and it is the point of the version:

  * the accent field is sampled at each voice's OWN step index. dois_12 through dois_17
    rolled it by the shift amount for a canon voice, so C carried A's weather displaced
    13 steps and was never under the same weather as A and B — against Principle III.
    84 of C's 108 notes (78%) change velocity. No onset, length or pitch moves, and
    A, B and D are untouched. verify() now ASSERTS one weather: voices sharing a grid
    must carry the same velocity wherever they strike together.

  * still open, deliberately untouched: the velocity RANKING table is per-voice, so the
    same accent combination can mean different velocities in D than in A/B/C. That is a
    separate decision and changing it here would confuse two variables.

New in dois_17 — two bug fixes, both found by GPT in its review (its own fixes live in
dois_16(gpt); these are written independently, keeping the two lines distinct). The
MUSIC is unchanged: every note is identical to dois_16.

  * the canon check reads each voice in its OWN steps. dois_16 divided both voices'
    onsets by the follower's unit, so giving C a different grid from A (160 ticks)
    crashed with KeyError: 15 — after the files had been written. It now also asserts
    that the canon moves by the interval the lattice predicts, not just some constant.
  * pitch settings must be whole numbers, checked before any output is replaced.
    PITCH_ROOT = 36.5 used to pass preflight, get truncated, and fail only afterwards.

Still true, and worth knowing: verify() reads the files back, so any problem only IT
can see is found after the previous files are replaced. The preflight checks cover
what can be known in advance; git keeps every committed render.
"""
import os
import glob
import hashlib
...
def check_pitch_lattice(note_layers):
    ...
    """
    # Whole numbers only, checked FIRST. A pitch is a MIDI note number; 36.5 passed
    # every range check below, was truncated to 36 when written, and failed only in
    # verify() — after the previous files had been replaced. (Found by GPT.) The test is
    # type() rather than isinstance(): Python counts True as the integer 1.
    for label, value in (('PITCH_ROOT', PITCH_ROOT),
        ...
        if type(value) is not int:
            raise ValueError(f"{label} must be a whole number, got {value!r} "
                             f"({type(value).__name__}); MIDI notes are integers and a "
                             f"fraction or True/False would be silently converted")
    row, col = lattice_axes()
    period = row * col
...
def verify(voices, periods, total_ticks, note_layers, base_binaries):
    ...
    # ONE WEATHER, asserted from the files rather than described. Voices sharing a grid
    # read the same accent field at the same step and rank it with the same table, so
    # wherever two of them strike together they must carry the SAME velocity. This is
    # Principle III made checkable. dois_12-dois_17 fail it: the accent field was rolled
    # for a canon voice, and A and C agreed at only 10 of their 80 shared attacks.
    by_grid = {}
    ...
    # read from the files and reported, so they cannot be confused again.
    #
    # A canon is a relationship between STEPS, not ticks: C's step i+13 answers A's
    # step i whatever grid each is played on. So each voice is read in its own steps —
    # its onsets divided by its OWN unit. dois_16 divided both voices by the follower's
    # unit, which is only right when they share one; giving C a 160-tick grid made it
    # crash with KeyError after the files were written. (Found by GPT, dois_16(gpt).)
    #
    # Asserted: every model attack has its answer and there are no extras, and the
    # modular interval is the one the lattice PREDICTS — diagonal x shift, mod period —
    # not merely some constant. (The prediction check is also GPT's idea.)
    ...
    for cfg in INSTRUMENT_CONFIGS:
        ...
        period, shift = note_layers[name], cfg['shift_amount']

        def one_layer(voice):
            """{step: pitch} for the first note layer of a voice, in its own steps."""
            unit = units[voice]
            path = os.path.join(OUTPUT_DIR, f"{TITLE}_{voice}_prime.mid")
            return {o // unit: p for o, _, p, _ in read_track(path)[0]
                    if o // unit < period}

        model, follower = one_layer(source), one_layer(name)
        answers = {i: (i + shift) % period for i in model}
        unanswered = sorted(i for i, j in answers.items() if j not in follower)
        unexpected = sorted(set(follower) - set(answers.values()))
        check(not unanswered and not unexpected,
              f"{name}: canon on {source} does not line up — model steps without an "
              f"answer {unanswered}, answering steps with no model {unexpected}")
        if unanswered or unexpected:
            continue
        heard = [follower[answers[i]] - model[i] for i in sorted(model)]
        modular = {d % period for d in heard}
        predicted = (PITCH_ROW_INTERVAL + PITCH_COL_INTERVAL) * shift % period
        check(modular == {predicted},
              f"{name}: canon on {source} moves by {sorted(modular)} mod {period}; the "
              f"lattice predicts +{predicted}")
        tally = lambda xs: ", ".join(f"{k:+d} x{v}" for k, v in
                                     sorted(Counter(xs).items(), key=lambda kv: -kv[1]))
        print(f"  canon {name} = {source} +{shift} steps "
              f"({units[source]}- and {units[name]}-tick grids): +{predicted} mod {period} "
              f"(exact, as predicted); heard {tally(heard)} semitones; "
...
def main():
    ...
    for cfg in INSTRUMENT_CONFIGS:
        ...
        # The accent field is sampled at the voice's OWN step index, always.
        #
        # dois_12 through dois_17 rolled it by the shift amount for a canon voice, with
        # no comment and no principle calling for it. The effect: C carried A's weather
        # displaced 13 steps, so at any moment C was under different weather from A and
        # B — which contradicts Principle III, the weather being one field every voice
        # falls under. 84 of C's 108 notes (78%) change velocity without the roll; not
        # one onset, length or pitch moves. verify() now asserts the principle directly,
        # so the roll cannot come back unnoticed.
