"""Regression tests for dois_18: the dois_16 bugs stay fixed, and the weather is one.

Every render goes into a temporary directory; nothing here touches dois_18/mid.
Run from dois_18:  python3 -B -m unittest discover -s tests -v
"""
import subprocess
import sys
import tempfile
...
PARENT = PROJECT.parent / 'dois_17' / 'mid'
...
class Dois18(unittest.TestCase):

    def test_only_c_velocities_differ_from_dois_17(self):
        """The weather fix must move no note: same onsets, lengths, pitches, channels."""
        with tempfile.TemporaryDirectory() as tmp:
            result = render(tmp)
            self.assertEqual(result.returncode, 0, result.stdout[-2000:] + result.stderr)
            # Key each note by what identifies it, so a changed velocity cannot
            # reorder same-onset notes and masquerade as a structural difference.
            def by_identity(path):
                return {(i, o, d, p, c): v for i, tr in enumerate(notes(path))
                        for o, d, p, v, c in tr}
            changed = {}
            for f in FILES:
                fresh = by_identity(Path(tmp) / f'dois_18_{f}.mid')
                prior = by_identity(PARENT / f'dois_17_{f}.mid')
                self.assertEqual(sorted(fresh), sorted(prior), f)   # same notes, same places
                changed[f] = sum(fresh[k] != prior[k] for k in fresh)
            self.assertEqual((changed['A_prime'], changed['B_prime'], changed['D_prime']),
                             (0, 0, 0))
            self.assertEqual(changed['C_prime'], 84)          # 84 of 108, 78%

    def test_one_weather_voices_on_a_grid_agree(self):
        """Principle III, from the files: A/C and B/C must agree wherever both strike."""
        with tempfile.TemporaryDirectory() as tmp:
            result = render(tmp)
            self.assertEqual(result.returncode, 0, result.stderr)
            at = {v: {o // 120: vel for o, _, _, vel, _ in
                      notes(Path(tmp) / f'dois_18_{v}_prime.mid')[0]} for v in 'ABC'}
            ...
            self.assertIn('one weather holds: A and C agree at all 80', result.stdout)
    ...
    def test_bug1_canon_across_different_grids(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (c_notes,) = notes(Path(tmp) / 'dois_18_C_prime.mid')
