...
def verify(voices, periods, total_ticks, note_layers, base_binaries):
    ...
    for cfg in INSTRUMENT_CONFIGS:
        ...
        unit, period = units[name], note_layers[name]
        at = lambda nm: {o // unit: p for o, _, p, _ in read_track(
            os.path.join(OUTPUT_DIR, f"{TITLE}_{nm}_prime.mid"))[0]}
        model, follower = at(source), at(name)
        span = periods[name] // unit
        heard = [follower[(i + cfg['shift_amount']) % span] - model[i]
                 for i in range(period) if i in model]
        modular = {d % period for d in heard}
        check(len(modular) == 1,
              f"{name}: canon on {source} is not a constant transposition mod {period}")
