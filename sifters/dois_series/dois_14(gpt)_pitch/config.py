...
# MIDI note 60 is middle C. Each voice has its own channel (displayed 1-based).
# Gaps are semitone intervals; C membership prefers up, absence down.
# D ranks pitch classes in its own layer. See MUSICAL_DESIGN.md.
PITCH_CONFIG = {
    'A': {'root': 48, 'motion': 'gaps', 'channel': 1},
    'B': {'root': 48, 'motion': 'gaps', 'channel': 2},
    'C': {'root': 48, 'motion': 'canon', 'channel': 3},
    'D': {'root': 36, 'motion': 'anchors', 'channel': 4},
}
...
def initial_pitch_settings(preset='creative'):
    """Reproduce the initial pitch mapping without changing the rhythm preset."""
    result = settings(preset)
    result['TITLE'] += '_initial'
    for name, motion in zip('ABCD', ('ascending', 'descending', 'canon', 'slow')):
        result['PITCH_CONFIG'][name]['motion'] = motion
    return result
