...
def check_velocity_table_is_shared(check, voices, periods):
    ...
    """
    # Reconstruct expected ranks from configured fields using exact counts, without
    # calling the writer's shared_velocity_levels or relying on its cached result.
    observations = []
    ...
    for name, _, _, unit in voices:
        ...
        weights = [1 - Fraction(int(sum(firing[label])), span) for label in labels]
        if weather_weights is None:
            weather_weights = weights[:-1]
        check(weather_weights == weights[:-1],
              f"{name}: weather densities differ from the shared field")
        span_weights.append(weights[-1])
        ...
        for onset, _, _, velocity in notes:
            ...
            observations.append((code, name, onset, velocity))

    weights = weather_weights + [max(span_weights)]
    order = sorted(range(1 << len(weights)), key=lambda code: (
        sum(w for bit, w in enumerate(weights) if code >> bit & 1), code))
    expected = {code: round(MIN_VELOCITY + Fraction(
        (MAX_VELOCITY - MIN_VELOCITY) * rank, len(order) - 1))
        for rank, code in enumerate(order)}

    # Retain EVERY occurrence: a later correct value must not hide an earlier wrong
    # one, even within a single voice. Keep diagnostics bounded, not the validation.
    table = {}
    wrong = []
    for code, name, onset, velocity in observations:
        table.setdefault(code, set()).add(velocity)
        if velocity != expected[code]:
            wrong.append((name, onset, code, velocity, expected[code]))
    clashes = {code: sorted(values) for code, values in table.items() if len(values) > 1}
    check(not clashes,
          f"accent states have inconsistent velocities across occurrences: {clashes}")
    check(not wrong,
          f"{len(wrong)} note(s) violate the shared velocity table; "
          f"first (voice, onset, state, actual, expected): {wrong[:5]}")
    if not clashes and not wrong:
        print(f"  one velocity table: {len(table)} accent states reached, "
              f"each meaning one velocity in every voice; every occurrence verified")
