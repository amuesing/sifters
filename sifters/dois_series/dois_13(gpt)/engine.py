...
def validate_settings(settings):
    expected = set('TITLE TICKS_PER_QUARTER_NOTE TEMPO_BPM DURATION_MULTIPLIER_KEY MIN_VELOCITY MAX_VELOCITY UNACCENTED_VELOCITY GATE_RATIO DRUM_RACK_BASE MAX_METER_NUMERATOR METER_OVERRIDE ACCENT_PHASE_POLICY VELOCITY_POLICY WEATHER SPAN_RESIDUE_SOURCE INSTRUMENT_CONFIGS MAX_STEPS MAX_EVENTS MAX_VOICES MAX_ACCENTS'.split())
    if not isinstance(settings, dict):
        raise ValueError('Settings must be a mapping')
    unknown, missing = set(settings) - expected, expected - set(settings)
    if unknown or missing:
        raise ValueError(f'Invalid settings: unknown={sorted(map(str, unknown))}, missing={sorted(missing)}')
    s = deepcopy(settings)
    ...
    if s['VELOCITY_POLICY'] not in ('per_grid_rarity', 'shared_rarity'):
        raise ValueError('VELOCITY_POLICY must be per_grid_rarity or shared_rarity')
...
def validate_grid(grid, layer, name):
    ...
    if len(set(passes)) != len(passes):
        ...
        for i, pattern in enumerate(passes, 1):
            if pattern in seen and len(duplicates) < 8:
                duplicates.append((seen[pattern], i))
            seen.setdefault(pattern, i)
        raise ValueError(f'{name}: repeated complete rhythm passes {duplicates}; span accent is not sufficient')
    if minimal_period(grid) != len(grid):
        raise ValueError(f'{name}: rendered pattern has a shorter musical period')


def derive_span_accent(layer_steps, span_steps, settings, name):
    modulus = required_modulus(layer_steps, span_steps)
    if modulus == 1:
        return None
    residues = [r for r in settings['SPAN_RESIDUE_SOURCE'] if r < modulus]
    if not residues:
        raise ValueError(f'{name}: no source residue fits span modulus {modulus}')
    expression = '|'.join(f'{modulus}@{r}' for r in sorted(residues))
    layer = sieve_layer(expression, settings['MAX_STEPS'])
    if len(layer) != modulus:
        raise ValueError(f'{name}: span residues reduce the required modulus {modulus}')
    return f'span{modulus}', expression, layer


def rarity(layer):
    return 1 - Fraction(sum(layer), len(layer))


def velocity_table(weights, settings):
    if not weights:
        return {0: settings['UNACCENTED_VELOCITY']}
    states = 1 << len(weights)
    if states > settings['MAX_VELOCITY'] - settings['MIN_VELOCITY'] + 1:
        raise ValueError('Too many accent states for distinct MIDI velocities')
    order = sorted(range(states), key=lambda code: (sum(w for bit, w in enumerate(weights)
                                                       if code & (1 << bit)), code))
    return {code: round(Fraction(settings['MIN_VELOCITY']) +
                        Fraction((settings['MAX_VELOCITY'] - settings['MIN_VELOCITY']) * rank, states - 1))
...
def build_plan(settings):
    ...
    span_accents = {name: derive_span_accent(len(layer), parity // units[name], s, name)
                    for name, layer in bases.items()}
    shared_weights = [rarity(a) for a in weather.values()]
    span_weights = [rarity(accent[2]) for accent in span_accents.values() if accent is not None]
    if span_weights:
        # A span has the same expressive meaning everywhere, regardless of its
        # grid's required modulus. The rarest span supplies one ensemble weight.
        shared_weights.append(max(span_weights))
    voices = []
    for cfg in s['INSTRUMENT_CONFIGS']:
        name = cfg['name']; layer = bases[name]; unit = units[name]; span = parity // unit
        accent_defs = tuple(s['WEATHER'].items())
        layers = list(weather.values())
        span_accent = span_accents[name]
        if span_accent is not None:
            label, expression, span_layer = span_accent
            accent_defs += ((label, expression),)
            layers.append(span_layer)
        if math.lcm(len(layer), *(len(a) for a in layers)) * unit != parity:
            raise ValueError(f'{name}: accent span does not equal first parity')
        weights = [rarity(a) for a in layers]
        if layers and s['VELOCITY_POLICY'] == 'shared_rarity':
            weights = shared_weights
        levels = velocity_table(weights, s)
...
def diagnostics(plan):
    """Describe realized accents at sounding onsets; never modify the plan."""
    settings = json.loads(plan.configuration_json)
    result = {'parity_ticks': plan.parity, 'seconds': plan.parity * plan.tempo / (plan.tpq * 1_000_000),
              'accent_phase_policy': settings['ACCENT_PHASE_POLICY'],
              'velocity_policy': settings['VELOCITY_POLICY'], 'voices': []}
    for voice in plan.voices:
        width = len(voice.layer)
        passes = [voice.velocities[i:i + width] for i in range(0, len(voice.velocities), width)]
        codes = {velocity: code for code, velocity in voice.levels}
        counts = {}
        for note in voice.notes:
            code = codes[note.velocity]
            counts[code] = counts.get(code, 0) + 1
        result['voices'].append({
            'name': voice.name, 'step_ticks': voice.unit, 'raw_period_ticks': width * voice.unit,
            'layer_attacks': sum(voice.layer), 'active_steps': [i for i, on in enumerate(voice.layer) if on],
            'accent_phase_steps': voice.phase, 'passes': len(passes), 'distinct_passes': len(set(passes)),
            'accent_labels': [label for label, _ in voice.accents],
            'velocity_histogram': {str(v): sum(n.velocity == v for n in voice.notes)
                                   for v in sorted({n.velocity for n in voice.notes})},
            'realized_states': [{'code': code, 'active_accents': [label for bit, (label, _) in enumerate(voice.accents)
                                                               if code & (1 << bit)],
                                 'velocity': dict(voice.levels)[code], 'notes': count}
                                for code, count in sorted(counts.items())],
            'unused_state_codes': sorted(set(dict(voice.levels)) - set(counts)),
        })
    return result
