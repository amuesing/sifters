...
def main(argv=None):
    ...
    group.add_argument('--diagnostics', action='store_true', help='print musical diagnostics as JSON without writing')
    ...
    try:
        ...
        if args.diagnostics:
            print(json.dumps(engine.diagnostics(plan), indent=2))
            return 0
        ...
        if args.verify_only:
            ...
            browser_output = midi.export_browser_files(plan, output, output.parent / (output.name + '-files'))
            print(f'MIDI for Ableton: {browser_output}')
            print(f'Verified generation and history: {output}')
        return 0
