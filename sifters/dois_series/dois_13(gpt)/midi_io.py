...
def _publish(plan, output):
    ...
    try:
        ...
        # Reuse only a completely verified current generation, including provenance.
        # A damaged generation must be repaired, even with an unchanged configuration.
        if previous is not None:
            try:
                verify_files(plan, previous)
                verify_manifest(plan, previous)
...
def export_browser_files(plan, source, destination):
    """Expose verified MIDI as ordinary files for DAW folder indexing.

    Each replacement is atomic; the set of six replacements is not a transaction.
    The managed source generation remains the authoritative complete snapshot.
    """
    source = Path(source).resolve(strict=True)
    destination = Path(destination)
    if destination.is_symlink() or (destination.exists() and not destination.is_dir()):
        raise ValueError('Browser export must be an ordinary directory')
    verify_files(plan, source)
    verify_manifest(plan, source)
    destination.mkdir(parents=True, exist_ok=True)
    names = tuple(expected_files(plan))
    for name in names:
        path = destination / name
        if path.is_symlink() or (path.exists() and not path.is_file()):
            raise ValueError(f'Browser export would replace a non-regular file: {path}')
    with tempfile.TemporaryDirectory(prefix='.browser-stage-', dir=destination.parent) as temporary:
        ...
        for name in names:
            path = destination / name
            if path.is_file() and path.read_bytes() == (stage / name).read_bytes():
                continue
            os.replace(stage / name, path)
    verify_files(plan, destination)
    return destination
