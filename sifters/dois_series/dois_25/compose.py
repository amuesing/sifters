...
from accents import (create_accent_binaries, duplicate_passes, notes_for_steps,
                     shared_velocity_levels, span_accent, step_velocities,
                     velocity_profile, voice_span)
...
def search_residue_source(field, note_layers, largest=16, most=5):
    ...
    for size in range(2, most + 1):
        for candidate in itertools.combinations(range(largest), size):
            try:
                fields = {c['name']: field(c, candidate) for c in INSTRUMENT_CONFIGS}
                levels = shared_velocity_levels({n: f['accents'] for n, f in fields.items()})
                if all(not duplicate_passes(velocities_of(f, levels), note_layers[n])
...
def velocities_of(field, levels):
    ...
    return step_velocities(field['rhythm'], field['accents'],
                           velocity_profile(field['accents'], levels))


def build_voices(fields, notes_for, note_layers):
    """Assemble each voice for one pitch mode. Only the notes differ between modes."""
    return [(cfg['name'],
             notes_for_steps(fields[cfg['name']]['velocities'],
                             notes_for(cfg, note_layers)),
             fields[cfg['name']]['velocities'],
             get_step_ticks(cfg))
            for cfg in INSTRUMENT_CONFIGS]
...
def require_distinct_passes(fields, note_layers, field):
...
def report_span_suggestion(fields, note_layers, field):
...
def prepare_accents(field, note_layers):
    ...
    """
    fields = {cfg['name']: field(cfg) for cfg in INSTRUMENT_CONFIGS}
    for cfg in INSTRUMENT_CONFIGS:
        got = fields[cfg['name']]
...
def main():
    ...
    fields, levels = prepare_accents(field, note_layers)
    if '--suggest-span' in sys.argv:
        report_span_suggestion(fields, note_layers, field)
        return
    require_distinct_passes(fields, note_layers, field)
    ...
    described = {mode: MODES[mode][1](note_layers) for mode in PITCH_MODES}
