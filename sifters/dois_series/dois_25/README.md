...
Every pitch mode is validated before any of them writes, so a setting that cannot
render leaves the previous files untouched.
...
| | |
|---|---|
| `mid/dois_25_static_*` | one fixed note per voice — A=36, B=37, C=38, D=39. Pitch carries nothing; the sieve speaks through rhythm and velocity. **This is the Moog Grandmother set**, which cannot take pitch as a CV source. |
| `mid/dois_25_lattice_*` | pitch read off the sieve's own 8x5 grid, MIDI 36-75. Same rhythm, same velocities — only the pitch differs. |
