"""Tests for dois_23: one velocity table for every voice, and not tied to one sieve.

Two contracts:
  1. the only thing that changed from dois_22 is voice D's velocities — the shared
...
Nothing here touches dois_22/mid.
"""
import re
import shutil
import subprocess
...
PRIOR = PROJECT.parent / 'dois_22' / 'mid'
...
class SameResult(unittest.TestCase):
    ...
    def test_only_voice_d_velocities_changed_from_dois_22(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            changed = {}
            for f in FILES:
                fresh = notes(root / 'mid' / f'dois_23_{f}.mid')
                prior = notes(PRIOR / f'dois_22_{f}.mid')
    ...
    def test_one_accent_state_means_one_velocity_everywhere(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for voice, unit in units.items():
                ...
                (track,) = notes(root / 'mid' / f'dois_23_{voice}_prime.mid')
