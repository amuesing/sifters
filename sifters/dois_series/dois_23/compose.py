...
def accent_field(cfg, base_binaries, note_layers, units, parity,
                 residue_source=None, levels=None):
    ...
    if levels is not None:
        ...
        field['notes_per_step'], field['velocities'] = accent_voicing(
            binary_full, accent_bins, profile, cfg['pitch'])
    return field


def build_voices(field, note_layers, levels):
    """Each voice's sounding steps and velocities, with its accents printed."""
    voices = []
    for cfg in INSTRUMENT_CONFIGS:
        name = cfg['name']
        got = field(cfg, levels=levels)
...
def main():
    ...
    def field(cfg, residue_source=None, levels=None):
        return accent_field(cfg, base_binaries, note_layers, units, parity,
                            residue_source, levels)
    ...
    # same velocity in every voice. Built from every voice's accents at once, which is
    # why the accents are gathered before any of them is voiced.
    levels = shared_velocity_levels({c['name']: field(c)['accents']
                                     for c in INSTRUMENT_CONFIGS})
    print(f"  velocity table, shared by every voice: "
          + " ".join(str(v) for _, v in sorted(levels.items())))

    check_static_pitch()

    print()
    voices = build_voices(field, note_layers, levels)
