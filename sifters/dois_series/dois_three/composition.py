...
def process_instrument(config, base_binary, period, ensemble_tracks):
    ...
    # For canon instruments, roll accent_binaries by the same amount as the
    # binary so the dynamic contour follows the shifted pattern.
...
def main():
    ...
    # Pass 1: build all binaries in config order. Derived instruments reference
    # previously built binaries by name, so order in INSTRUMENT_CONFIGS matters.
    base_binaries = {}
    for config in INSTRUMENT_CONFIGS:
        binary, period = build_binary(config, base_binaries)
        base_binaries[config['name']] = binary

    # Pass 2: render each instrument's full clip set.
    ensemble_tracks = []
    for config in INSTRUMENT_CONFIGS:
