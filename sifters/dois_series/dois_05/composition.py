...
def build_form_plan(form_sieves, num_sections):
    presence = {}
    for name, pattern in form_sieves.items():
        s = music21.sieve.Sieve(pattern)
        s.setZRange(0, num_sections - 1)
        presence[name] = sieve_to_binary(s)

    plan = []
    for section_idx in range(num_sections):
        active = {name for name, binary in presence.items() if binary[section_idx]}
        plan.append(active)

    print("Form plan (macro-sieve presence per section):")
    for i, active in enumerate(plan):
        print(f"  section {i}: {sorted(active)}")

    return plan

def create_arrangement_midi(ensemble_tracks, form_plan, filename='arrangement'):
    if not ensemble_tracks:
        return
    ...
    for i, (name, notes_per_step, velocities, step_ticks, time_signature) in enumerate(ensemble_tracks):
        ...
        for section_idx, active_names in enumerate(form_plan):
            if name not in active_names:
                continue
            section_start = section_idx * section_length_ticks
    ...
    total_ticks = section_length_ticks * len(form_plan)
    print(f"Arrangement: {len(form_plan)} sections x {section_length_ticks} ticks = {total_ticks} ticks total "
...
def main():
    ...
    # Pass 1: build every binary, including non-rendering helpers.
    base_binaries = {}
    for config in INSTRUMENT_CONFIGS:
        binary, period = build_binary(config, base_binaries)
        base_binaries[config['name']] = binary

    # Pass 2: render the per-clip library (unchanged workflow for manual
    # arrangement in Ableton's session view).
    ensemble_tracks = []
    for config in INSTRUMENT_CONFIGS:
    ...
    # Pass 3: macro-form. Evaluate the form sieves into a per-section
    # presence map, then render one composed arrangement file spanning the
    # whole piece, ready to drop into Ableton's arrangement view.
    form_plan = build_form_plan(FORM_SIEVES, NUM_SECTIONS)
    create_arrangement_midi(ensemble_tracks, form_plan, filename=f"{TITLE}_arrangement")
