...
def state_ceilings(base_binaries, weather, note_layers):
    ...
    or not on each of them, its ceiling is twice that count. B is the case that matters
    here: it lands on only three of the four weather combinations, so two states can
    never sound in it however the span accent is chosen. Knowing the ceiling is what
    lets the search tell "as many as possible" from "not all eight".
    """
...
def derive_span_residues(field, note_layers, ceilings, largest=16, most=4):
    """The span accent's residues, searched rather than written by hand.

    The span accent's first job is to make every restatement parity forces sound
    different, so a candidate is only viable if no two passes of any voice come out
    identical. Among the viable ones the search wants the velocity states USED — all of
    them, in every voice, as far as each voice's rhythm allows. Evenness is not the
    goal: a state may sound once or fifty times, but a state that never sounds is a
    level of the shared table the piece never reaches.

    Candidates are tried smallest first, then in order. The first that puts every voice
    at its ceiling wins outright, since nothing can beat it. Otherwise the best total is
    kept, and the shortfall is reported rather than hidden.
    """
    best = None
    for size in range(1, most + 1):
        for candidate in itertools.combinations(range(largest), size):
            ...
            used = {n: len({int(v) for v in velocities_of(f, levels) if v})
                    for n, f in fields.items()}
            if all(used[n] >= ceilings[n] for n in used):
                return candidate, used
            if best is None or sum(used.values()) > sum(best[1].values()):
                best = (candidate, used)
    if best is None:
        raise ValueError(
            "no span-accent residues up to four members leave every pass distinct for "
...
def main():
    ...
    if config.SPAN_RESIDUE_SOURCE:
        ...
        source, used = derive_span_residues(field, note_layers, ceilings)
        short = {n: (used[n], ceilings[n]) for n in used if used[n] < ceilings[n]}
        print(f"  span residues (derived): {source} — every pass distinct; velocity "
              f"states used " + ", ".join(f"{n} {used[n]}/{ceilings[n]}" for n in used)
              + ("" if not short else
                 f"  (short in {', '.join(short)}: no residues reached their ceiling)"))
