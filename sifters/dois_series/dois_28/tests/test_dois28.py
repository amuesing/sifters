"""Tests for dois_28: two pitch modes from one rhythm, and not tied to one sieve.
...
Nothing here touches dois_28/mid.
"""
import json
import re
import shutil
...
class SameResult(unittest.TestCase):
    ...
    def test_static_mode_keeps_the_drumrack_rhythm_and_pitches(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for f in FILES:
                fresh = notes(root / 'mid' / f'dois_28_static_{f}.mid')
    ...
    def test_the_weather_is_derived_from_the_sieve(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('velocity states used', result.stdout)

    def test_every_voice_uses_every_velocity_state_it_can(self):
        """All states used, as far as each voice's rhythm allows. Evenness is not asked.

        B only lands on three of the four weather combinations, so two of the eight
        states can never sound in it; its ceiling is six, and it must reach six.
        """
        ceilings = {'A': 8, 'B': 6, 'C': 8, 'D': 8}
        with tempfile.TemporaryDirectory() as tmp:
            root = project_copy(tmp)
            result = run(root)
            self.assertEqual(result.returncode, 0, result.stderr)
            for voice, ceiling in ceilings.items():
                (track,) = notes(root / 'mid' / f'dois_28_static_{voice}_prime.mid')
                used = {v for _, _, _, v, _ in track}
                self.assertEqual(len(used), ceiling, f'{voice} used {sorted(used)}')
            self.assertIn('A 8/8, B 6/6, C 8/8, D 8/8', result.stdout)
    ...
    def test_lattice_shares_the_rhythm_and_velocities_of_static(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run(root).returncode, 0)
            for f in FILES:
                a = notes(root / 'mid' / f'dois_28_static_{f}.mid')
                b = notes(root / 'mid' / f'dois_28_lattice_{f}.mid')
    ...
    def test_lattice_pitches_and_canon(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            at = {}
            for voice in 'ABCD':
                (track,) = notes(root / 'mid' / f'dois_28_lattice_{voice}_prime.mid')
