"""dois_19(gpt): stationary local accent weather and additive lattice-to-pitch-class mapping, forked from Claude dois_17.

Rhythm, accent policy, gates and original parity are retained. Pitch is a homomorphism
from the two cyclic lattice axes into Z/12. Positive generators are derived from gcd;
root and a single-octave realization are explicit compositional choices. See README.
"""
import os
import glob
import hashlib
...
def lattice_axes():
    ...
    with each axis movement derived by pitch_axis_intervals().
    """
...
def pitch_axis_intervals():
    """Smallest positive generator of the largest image each axis allows in Z/12.

    An axis of order m permits gcd(m,12) classes. When that number is one, the only
    class movement is zero. No manually selected scale or axis interval is supplied.
    """
    return tuple(12 // math.gcd(m, 12) if math.gcd(m, 12) > 1 else 0
                 for m in lattice_axes())


def lattice_pitch(step, period):
    """Read both lattice coordinates, then realize the resulting class above root."""
    row, col = lattice_axes()
    a, b = pitch_axis_intervals()
    return PITCH_ROOT + (a * (step % row) + b * (step % col)) % 12


def check_pitch_lattice(note_layers):
    """Validate cyclic/additive closure into pitch classes, not 40-pitch bijectivity."""
    if type(PITCH_ROOT) is not int:
        raise ValueError('PITCH_ROOT must be a whole number, not a fraction or boolean')
    row, col = lattice_axes()
    period = row * col
    if any(p != period for p in note_layers.values()):
        raise ValueError('Every note layer must match the product of the lattice axes')
    a, b = pitch_axis_intervals()
    if row * a % 12 or col * b % 12:
        raise ValueError('Pitch-class movement does not close on its lattice axis')
    diagonal = a + b
    reached = set()
    for n in range(period):
        pitch = lattice_pitch(n, period)
        if pitch != PITCH_ROOT + diagonal * n % 12:
            raise ValueError('Lattice and additive pitch-class forms disagree')
        if not 0 <= pitch <= 127:
            raise ValueError('Pitch-class realization exceeds MIDI 0–127; adjust root')
        reached.add((pitch - PITCH_ROOT) % 12)
    if len(reached) != math.gcd(period, 12):
        raise ValueError('Pitch-class map does not reach its maximum possible image')
    return diagonal
...
def verify(voices, periods, total_ticks, note_layers, base_binaries):
    ...
    for name, _, velocities, step_ticks in voices:
        ...
        diagonal = sum(pitch_axis_intervals())
        wrong = [(o, p) for o, _, p, _ in notes
                 if p != PITCH_ROOT + (diagonal * ((o // step_ticks) % period)) % 12]
    ...
    # Canon in local step coordinates, with transposition predicted modulo 12.
    # The single-octave realization can wrap by an octave; report both actual and
    # pitch-class intervals. Retain dois_17's unequal-grid and correspondence guards.
    ...
    for cfg in INSTRUMENT_CONFIGS:
        ...
        modular = {d % 12 for d in heard}
        predicted = sum(pitch_axis_intervals()) * shift % 12
        check(modular == {predicted},
              f"{name}: canon on {source} moves by {sorted(modular)} mod 12; the "
        ...
        print(f"  canon {name} = {source} +{shift} steps "
              f"({units[source]}- and {units[name]}-tick grids): +{predicted} mod 12 "
...
def main():
    if ACCENT_PHASE_POLICY != 'fixed_local_steps':
        raise ValueError('This comparison requires fixed_local_steps accent phase')
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ...
    row, col = lattice_axes()
    print(f"\n  Pitch-class lattice {row} x {col}: axis movements {pitch_axis_intervals()}, "
          f"diagonal {diagonal} mod 12; {math.gcd(row * col, 12)} classes, "
          f"root {PITCH_ROOT}; pitch period {12 // math.gcd(diagonal, 12)} local steps")
    ...
    for cfg in INSTRUMENT_CONFIGS:
        ...
        # Rhythm and pitch retain their canon; all accent fields stay at local i.
