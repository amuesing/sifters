...
# ---------------------------------------------------------------------------
# PITCH — additive map from lattice axes into twelve pitch classes
# ---------------------------------------------------------------------------
# The renderer derives axis movements rather than configuring a scale:
# m * movement = 0 mod 12. Choose the smallest positive generator of the
# largest permitted image; use zero if only the trivial image is possible.
# Axes 8 and 5 therefore yield movements 3 and 0, with four relative classes.
# MIDI realization is root + class offset in one octave. This register and root
# are explicit choices, not information provided by the sieve.
PITCH_ROOT = 36

# Accent fields use unshifted LOCAL step indices, including the derived span accent.
# Different voice durations do not imply a shared wall-clock accent phase.
ACCENT_PHASE_POLICY = 'fixed_local_steps'
