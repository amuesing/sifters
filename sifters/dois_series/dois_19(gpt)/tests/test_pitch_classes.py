...
def fixed_velocity(i):
    # Independent clause/state arithmetic for C's unchanged 160-step accent span.
    weights=(Fraction(3,5),Fraction(3,8),Fraction(29,32))
    order=sorted(range(8),key=lambda code:(sum(weights[k] for k in range(3) if code&(1<<k)),code))
    state=int(i%5 in (1,3))+2*int(i%8 in (0,1,2,5,6))+4*int(i%32 in (0,1,7))
    return 1+18*order.index(state)
...
class PitchClassTests(unittest.TestCase):
    ...
    def test_six_files_preserve_parent_except_c_velocity(self):
        expected={}
        for name,unit in zip('ABCD',(120,120,120,160)):
            expected[name]=[[o,d,ch,36+3*((o//unit)%4),fixed_velocity(o//unit) if name=='C' else v] for o,d,ch,p,v in self.fixture[name]['notes']]
    ...
    def test_c_samples_stationary_weather_on_each_attack(self):
        notes=decode(self.out/f'{c.TITLE}_C_prime.mid')[0][0]
        self.assertTrue(all(n[4]==fixed_velocity(n[0]//120) for n in notes))
        shifted=[fixed_velocity(n[0]//120-13) for n in notes]
        self.assertNotEqual([n[4] for n in notes],shifted)
        for name in 'AB':
            other={n[0]:n[4] for n in decode(self.out/f'{c.TITLE}_{name}_prime.mid')[0][0]}
            for n in notes:
                if n[0] in other:self.assertEqual(n[4],other[n[0]])
