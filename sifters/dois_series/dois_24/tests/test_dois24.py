"""Tests for dois_24: two pitch modes from one rhythm, and not tied to one sieve.
...
class SameResult(unittest.TestCase):
    ...
    def test_lattice_shares_the_rhythm_and_velocities_of_static(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run(root).returncode, 0)
            for f in FILES:
                a = notes(root / 'mid' / f'dois_24_static_{f}.mid')
                b = notes(root / 'mid' / f'dois_24_lattice_{f}.mid')
    ...
    def test_lattice_pitches_and_canon(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            at = {}
            for voice in 'ABCD':
                (track,) = notes(root / 'mid' / f'dois_24_lattice_{voice}_prime.mid')
    ...
    def test_each_voice_sounds_one_static_pitch(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for voice, pitch in (('A', 36), ('B', 37), ('C', 38), ('D', 39)):
                (track,) = notes(root / 'mid' / f'dois_24_static_{voice}_prime.mid')
