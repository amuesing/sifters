"""dois_15(gpt) — Claude lattice baseline with explicit verification.
...
def config_fingerprint():
    ...
    project folder.
    """
    # Use effective values, excluding runtime-generated accent dictionaries.
    document = {
        'voices': [{k: v for k, v in c.items() if k != 'accent_dict'}
                   for c in INSTRUMENT_CONFIGS],
        'weather': WEATHER, 'span_residues': SPAN_RESIDUE_SOURCE,
        'tempo': TEMPO_BPM, 'ppq': TICKS_PER_QUARTER_NOTE,
        'gate': GATE_RATIO, 'velocity': [MIN_VELOCITY, MAX_VELOCITY, UNACCENTED_VELOCITY],
        'meter': [TIME_SIGNATURE, MAX_METER_NUMERATOR],
        'pitch': [PITCH_ROOT, PITCH_ROW_INTERVAL, PITCH_COL_INTERVAL],
        'pitch_mode': PITCH_MODE, 'pitch_cycles': PITCH_CYCLES,
    }
    return hashlib.sha256(json.dumps(document, sort_keys=True).encode()).hexdigest()[:12]
...
def lattice_pitch(step, period):
    """Pitch for a step, read off the sieve's own 8 x 5 lattice.

    See config.py for the geometry. Equivalent to PITCH_ROOT + (13*step) mod period,
    and asserted to be so at render time — the two forms are the same operation.
    """
    return PITCH_ROOT + (PITCH_ROW_INTERVAL * (step % 8)
                         + PITCH_COL_INTERVAL * (step % 5)) % period

def check_pitch_lattice(period):
    """The lattice is only a lattice if the map is a bijection on the period.

    gcd(multiplier, period) = 1 is what makes it one, and a bijection is what carries
    residue classes to residue classes — i.e. what makes the pitch set the rhythm sieve
    under a sieve-preserving transformation rather than an arbitrary reordering.
    """
    if period != 40:
        raise ValueError('This pitch lattice requires a 40-step layer with axes 8 and 5')
    for value in (PITCH_ROOT, PITCH_ROW_INTERVAL, PITCH_COL_INTERVAL):
        if isinstance(value, bool) or not isinstance(value, int):
            raise ValueError('Pitch root and lattice intervals must be integers')
    if not 0 <= PITCH_ROOT <= 88:
        raise ValueError('The full lattice must fit MIDI 0–127')
    step = PITCH_ROW_INTERVAL + PITCH_COL_INTERVAL
    if math.gcd(step, period) != 1:
        raise ValueError(f"the diagonal step {step} shares a factor with the {period}-step "
                         f"period, so the lattice map is not a bijection and some pitches "
                         f"would be unreachable while others repeat")
    reached = {lattice_pitch(n, period) for n in range(period)}
    if len(reached) != period:
        raise ValueError(f"lattice reaches only {len(reached)} of {period} pitches")
    for n in range(period):
        if lattice_pitch(n, period) != PITCH_ROOT + (step * n) % period:
            raise ValueError(f"lattice and multiplier disagree at step {n}")
    return step

def pitch_index(onset, unit, period, parity, name):
    """Read an independent clock at an attack; boundaries select the new state."""
    if PITCH_MODE == 'lattice':
        return (onset // unit) % period
    return (onset * period * PITCH_CYCLES[name] // parity) % period


def check_pitch_clocks(note_layers, units, parity):
    if PITCH_MODE not in ('lattice', 'moduli'):
        raise ValueError('Unknown pitch mode')
    if PITCH_MODE == 'lattice':
        return
    if set(PITCH_CYCLES) != set(note_layers):
        raise ValueError('Pitch cycles must specify exactly the rendered voices')
    durations = []
    for name, period in note_layers.items():
        cycles = PITCH_CYCLES[name]
        if isinstance(cycles, bool) or not isinstance(cycles, int) or cycles <= 0:
            raise ValueError('Pitch cycle counts must be positive integers')
        # Exact boundaries: no float time or rounded pitch steps.
        if parity % (period * cycles):
            raise ValueError(f'{name}: pitch steps must divide parity in integer MIDI ticks')
        durations.append(parity // cycles)
        assert pitch_index(parity, units[name], period, parity, name) == 0
    raw = [note_layers[n] * units[n] for n in note_layers]
    if math.lcm(*raw, *durations) != parity:
        raise ValueError('Pitch clocks change the first rhythmic parity')


def accent_voicing(binary, accent_binaries, profile, root_note, pitch_period, name, unit, parity):
    ...
    for i in np.flatnonzero(on):
        notes_per_step[i] = [lattice_pitch(pitch_index(int(i) * unit, unit, pitch_period, parity, name), pitch_period)]
...
def verify(voices, periods, total_ticks, note_layers, base_binaries):
    ...
    for name, _, velocities, step_ticks in voices:
        ...
        # recomputed here rather than compared against the value used to write it.
        period = note_layers[name]
        wrong = [(o, p) for o, _, p, _ in notes
                 if p != PITCH_ROOT + ((PITCH_ROW_INTERVAL + PITCH_COL_INTERVAL) *
                       ((o // step_ticks) if PITCH_MODE == 'lattice' else
                        (o * period * PITCH_CYCLES[name] // total_ticks))) % period]
    ...
    drumrack    = os.path.join(OUTPUT_DIR, f"{TITLE}_ensemble.mid")
    ...
    for name, _, _, _ in voices:
        ...
        drum, _, _ = read_track(drumrack, channel=channels[name])
...
def main():
    ...
    check_pitch_clocks(note_layers, units, parity)
    for layer in note_layers.values():
        diagonal = check_pitch_lattice(layer)
    print(f"\n  Pitch lattice: {PITCH_ROW_INTERVAL} semitones per mod-8 row, "
          f"{PITCH_COL_INTERVAL} per mod-5 column — diagonal step {diagonal}, "
          f"bijective on {note_layers[INSTRUMENT_CONFIGS[0]['name']]} pitches from "
          f"root {PITCH_ROOT}")
    ...
    for cfg in INSTRUMENT_CONFIGS:
        ...
        notes_per_step, velocities = accent_voicing(binary_full, accent_bins,
                                                    profile, cfg['root'],
                                                    note_layers[name], name, step_ticks, parity)
...
def cli(argv=None):
    global PITCH_MODE, TITLE, OUTPUT_DIR
    parser = argparse.ArgumentParser(description='Claude lattice baseline and separate modulus-clock study')
    parser.add_argument('--pitch-mode', choices=('lattice', 'moduli'), default='lattice')
    parser.add_argument('--output-dir', type=Path)
    args = parser.parse_args(argv)
    PITCH_MODE = args.pitch_mode
    TITLE = 'dois_15(gpt)' + ('_moduli' if PITCH_MODE == 'moduli' else '')
    OUTPUT_DIR = str(args.output_dir or (Path(__file__).resolve().parent / 'mid' / PITCH_MODE))
    try:
        main()
