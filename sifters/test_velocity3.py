...
def accent_velocity(binary, primary_modulo, secondary_modulo, primary_start=0, secondary_start=0, overlap_value=120, primary_value=100, secondary_value=80, normal_value=60):
    velocities = []
    for i, value in enumerate(binary):
        if value == 0:
            velocities.append(0)
        elif (i - primary_start) % primary_modulo == 0 and (i - secondary_start) % secondary_modulo == 0:
            velocities.append(overlap_value)
        elif (i - primary_start) % primary_modulo == 0:
            velocities.append(primary_value)
        elif (i - secondary_start) % secondary_modulo == 0:
...
for n, s in sieve_objs:
    ...
    for i in indices:
        ...
        velocities = accent_velocity(shifted_sieve, primary_modulo=8, secondary_modulo=5, primary_start=1)
        create_midi(shifted_sieve, largest_period, filename, velocities)

    original_sieve = prime_sieve(s)
    filename = f'{title}_{n}_prime.mid'
    velocities = accent_velocity(original_sieve, primary_modulo=8, secondary_modulo=5, primary_start=1)
    ...
    velocities = accent_velocity(inverted_sieve, primary_modulo=8, secondary_modulo=5, primary_start=1)
    ...
    velocities = accent_velocity(reversed_sieve, primary_modulo=8, secondary_modulo=5, primary_start=1)
    ...
    velocities = accent_velocity(stretched_sieve, primary_modulo=8, secondary_modulo=5, primary_start=1)
